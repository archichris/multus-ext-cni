...
func (nw *network) Run(ctx context.Context, receiver chan []Event) {
	wg := sync.WaitGroup{}
...
type vxlan struct{
	ID    string
	Attrs vxlanLeaseAttrs
	Route netlink.Route
	DirectRoute netlink.Route
}

var vxlans = []vxlanInfo{}
...
func subnetRemovedProc(vxlan *vxlan) error{
	if vxlan.directRoutingOK {
		log.V(2).Infof("Removing direct route to subnet: %s PublicIP: %s", sn, attrs.PublicIP)
		if err := netlink.RouteDel(&directRoute); err != nil {
			log.Errorf("Error deleting route to %v via %v: %v", sn, attrs.PublicIP, err)
		}
...
type vxlanManger struct{
	vxlans map[string]*network
}

func (m *vxlanManger) AppendNet(n *network) error{
	if _,ok :=m.vxlans[n.dev.vni];ok{
		return fmt.Errorf("vxlan %d exists", n.dev.vni)
	} 
	m.vxlans[n.dev.vni]=n 
	return nil
}

func (m *vxlanManger) DelNet(vni int32) eror{
	//todo finish the delete operation
	if _,ok :=m.vxlans[n.dev.vni];ok{
		delete(m.vxlans, vni)
	} 
}
