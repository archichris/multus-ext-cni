...
var (
	...
	macPrefixStr = "0a:0a"
	macPrefix    = []byte{0xa, 0xa}
	hostPrefix   = []byte{0xa, 0x64}
	guestPrefix  = []byte{0xc0, 0xa8}
)
...
func ensureAddr(br netlink.Link, family int, ipn *net.IPNet, forceAddress bool) error {
	return ensureAddrWithHW(br, family, ipn, br.Attrs().HardwareAddr, forceAddress)
}

func ensureAddrWithHW(br netlink.Link, family int, ipn *net.IPNet, hwaddr net.HardwareAddr, forceAddress bool) error {
...
func BridgeAdd(args *skel.CmdArgs, n *NetConf, masterIP net.IP) (*netlink.Bridge, *current.Result, error) {
	...
	if isLayer3 {
		...
		if err := netns.Do(func(_ ns.NetNS) error {
			...
			mac := IPToMac(masterIP, result.IPs[0].Address.IP)

			err = netlink.LinkSetHardwareAddr(l, IPToMac(masterIP, result.IPs[0].Address.IP))

			logging.Debugf("set mac:%v to args.IfName result:%v", mac.String(), err)
	...
	if debugPostIPAMError != nil {
		return nil, nil, debugPostIPAMError
	}
...
func BridgeDel(args *skel.CmdArgs, n *NetConf) error {
...
func BridgeCheck(args *skel.CmdArgs, n *NetConf) error {
	...
	if !brCNI.found {
		return fmt.Errorf("CNI created bridge %s in host namespace was not found", brName)
	}
	if !contCNI.found {
		return fmt.Errorf("CNI created interface in container %s not found", args.IfName)
	}
	if !vethCNI.found {
		return fmt.Errorf("CNI veth created for bridge %s was not found", brName)
	}
...
func IPToMac(host net.IP, guest net.IP) net.HardwareAddr {
	mac := append(macPrefix, []byte(host.To4())[2:]...)
	mac = append(mac, []byte(guest.To4())[2:]...)
	return (net.HardwareAddr)(mac)
}

func MacToIP(lladdr net.HardwareAddr) (net.IP, net.IP) {
	host := (net.IP)(append(hostPrefix, lladdr[2:4]...))
	guest := (net.IP)(append(guestPrefix, lladdr[4:6]...))
	return host, guest
}
